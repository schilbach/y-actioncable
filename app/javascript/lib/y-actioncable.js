...
const sendActionCableMessage = (provider, action, payload) => {
    if (provider.actionCableIdentifier && provider.wsconnected) {
        const message = {
            command: 'message',
            identifier: provider.actionCableIdentifier,
            data: JSON.stringify({
                action: action,
                payload: payload,
            }),
        };
        return /** @type {WebSocket} */ (provider.ws).send(JSON.stringify(message));
    }
}
...
const setupWS = provider => {
    if (provider.shouldConnect && provider.ws === null) {
        const websocket = new provider._WS(provider.actionCableUrl)
        ...
        websocket.onmessage = event => {
            ...
            if (data['type'] == 'confirm_subscription') {
                provider.actionCableIdentifier = data['identifier']
                ...
                sendActionCableMessage(provider, 'receive', encoding.toUint8Array(encoder))
                ...
                if (provider.awareness.getLocalState() !== null) {
                    ...
                    sendActionCableMessage(provider, 'receive', encoding.toUint8Array(encoderAwarenessState))
                }
                return
            }
            ...
            const encoder = readMessage(provider, new Uint8Array(data['message']), true)
        ...
        websocket.onclose = () => {
            ...
            provider.actionCableIdentifier = null
        ...
        websocket.onopen = () => {
            ...
            console.log('WebSocket is connected. Send channel and id ...');

            const msg = {
                command: 'subscribe',
                identifier: JSON.stringify({
                    content_id: provider.dataId,
            ...
            console.log('Sent channel and id ...');
...
const broadcastMessage = (provider, buf) => {
    sendActionCableMessage(provider, 'receive', buf)
 ...
 *   import { ActionCableProvider } from 'y-websocket'
 *   const doc = new Y.Doc()
 *   const provider = new ActionCableProvider('http://localhost:1234', 'my-document-name', doc)
 *
...
export class ActionCableProvider extends Observable {
     ...
     */
    constructor (channelName, dataId, doc, { connect = true, awareness = new awarenessProtocol.Awareness(doc), params = {}, WebSocketPolyfill = WebSocket, resyncInterval = -1 } = {}) {
        super()
        let serverUrl = 'ws://' + location.hostname + ':' + location.port + '/cable'
        while (serverUrl[serverUrl.length - 1] === '/') {
            serverUrl = serverUrl.slice(0, serverUrl.length - 1)
        }
        const encodedParams = url.encodeQueryParams(params)
        const roomname = channelName + '_' + dataId
        this.bcChannel = serverUrl + '/' + roomname
        this.url = serverUrl + '/' + roomname + (encodedParams.length === 0 ? '' : '?' + encodedParams)
        this.actionCableUrl = serverUrl
        this.dataId = dataId
        this.channelName = channelName
        this.actionCableIdentifier = null
        ...
        if (resyncInterval > 0) {
            this._resyncInterval = /** @type {any} */ (setInterval(() => {
                if (this.ws) {
                    ...
                    sendActionCableMessage(this, 'receive', encoding.toUint8Array(encoder))
                }
            }, resyncInterval))
        }
